which.inbreds <- function(summary, min.quality, list.id, probs, phen.code) {
  if(probs) {
    if (phen.code == 'plink') {
    ...
    if(missing(list.id)) { # pas de list.id : défaut 
      if(test) { # il y a des atteints
        ...
        w.HBD   <- which( summary$quality >= min.quality & summary$inbred )
        if (phen.code == 'plink') {
          w.HFLOD <- match( which(summary$quality >= min.quality & summary$inbred & summary$pheno == 2), w.HBD )
        } else {
          w.HFLOD <- match( which(summary$quality >= min.quality & summary$inbred & summary$pheno == 1), w.HBD )
        }
      } else {
